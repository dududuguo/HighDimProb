...
- Focused full Dudley API check: `HighDimProbTest/DudleyAPI.lean`; it consumes `dudleyEntropyIntegral` only through the public signature. Run `lake build HighDimProb.Concentration.Dudley` and then `lake build HighDimProbTest.DudleyAPI` when changing the endpoint.
