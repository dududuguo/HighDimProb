...
- Focused finite-chaining and metric-entropy API checks: `HighDimProbTest/ChainingAPI.lean`, `HighDimProbTest/NetsMetricEntropyAPI.lean`, `HighDimProbTest/SumIntegralAPI.lean`, `HighDimProbTest/MetricEntropyAPI.lean`, and `HighDimProbTest/SubGaussianProcessAPI.lean`; they cover internal-net parent maps, finite endpoint paths, common-anchor residual/step-bound composition including the zero-level boundary, positive-radius net families, explicit `Nat` cardinality data, the generic arbitrary-partition sum/integral adapter, the truncated dyadic entropy comparison, and the D3 passage from uniformly bounded dense-sequence prefixes to the full anchored supremum. Run `lake build HighDimProbTest.SubGaussianMaxAPI HighDimProbTest.ChainingAPI HighDimProbTest.NetsMetricEntropyAPI HighDimProbTest.SumIntegralAPI HighDimProbTest.MetricEntropyAPI HighDimProbTest.SubGaussianProcessAPI HighDimProbJudge.Concentration.SubGaussianProcessUse` after changing these public surfaces. The judge checks cover the supplied single-path certificate, the D1 finite-family certificate, and the D3 supremum-passage contract. They are not evidence for a vanishing residual, total-bounded geometry construction, the small-scale/full-integral limit, or Dudley; open coverage follows [`TODO.md`](TODO.md).
