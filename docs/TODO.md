...
- Done: the rank-one nullspace examples now reuse the S6 core bridge where it
  removes local action/sum algebra.
- Done: RM-ON-S2/RM-ON-S3 proved the nonempty deterministic operator-norm
  witness and the nonempty
  `selfAdjointOperatorNormTailViaQuadraticFormStatement_nonempty` bridge.
- Done: RM-ON-S4 proved
  `matrixBernsteinSelfAdjointOperatorNormTailOptimizedScalarRHSWithBernsteinCoeff_nonempty_under_primitives`,
  removing the explicit spectral-bridge assumption from the nonempty
  operator-norm Matrix Bernstein wrapper while keeping Tropp, CFC,
  variance-proxy, independence, and integrability assumptions explicit.
- Done: RM-ON-S5 proved
...
- Done: RM-BR state prefix/suffix partial-sum API added finite-sum/state
  bookkeeping in `Sums.lean`: comparison and random-matrix prefix/suffix
  sums, endpoint/successor lemmas, and `randomMatrixSum_eq_prefixSum_last`.
  This prepares natural-state construction only; it does not prove Lieb,
...
- Done: RM-negative-exp-trace-primitive-audit adds sign-normalization adapters
  for negative-family matrix-exp, finite sums, trace-exp, and the Bernstein CFC
  statement.
- Done: RM-negative-tropp-primitive-boundary-audit adds second-moment,
  variance-proxy, K-family, MGF-comparison, and post-provider trace-MGF
  sign-normalization adapters for `negRandomMatrixFamily`.
...
| A.e.-nonnegative scalar predicate | Add a reusable predicate or convention for `0 <=?[P] X` when pointwise nonnegativity is too strong. | `Filter.Eventually`, `AEMeasurable`, `Integrable` | easy/medium | `markov_inequality_nonneg` uses pointwise nonnegativity to keep the first wrapper simple. |
...
| Metric entropy radius bridge lemmas | Relate HighDimProb real-radius wrappers to Mathlib `NNReal` / `NNReal? radii in theorem statements. | `epsilonRadius`, `epsilonERadius`, Mathlib metric APIs | medium | Stage P1 did not need extra bridge lemmas, but future covering-number proofs may. |
...
| Metric entropy real-log wrapper | Define a real-valued metric entropy convention from `coveringNumber`. | `coveringNumber`, `ENNReal`, `Real.log` | medium | Stage 5A keeps Mathlib `ENNReal` counts and defers finite/infinite conventions. |
...
| Isotropic squared-norm expectation | Prove isotropic random vectors satisfy `E ||X||^2 = n`. | `IsotropicSecondMoment`, `sqNorm`, finite sums | medium/hard | Requires summing diagonal second moments and expectation/sum interchange. |
...
| epsilon-net operator norm bound | Prove operator norm bounds using nets on spheres. | nets, sphere, matrix/operator norm APIs | hard | Stage MC2-fix proves the basic operator-norm comparison bridge; finite net approximation and sphere-cover arguments remain deferred. |
...
- Done: proved the narrow `Fin m` provider
  `troppMasterTraceMGFFiniteFamily_of_conditionalSteps` under explicit S5
  conditional-step/state data.
- Done: proved `traceMGFBernsteinVarianceProxyBound_of_troppConditionalSteps`
  as a public wrapper that derives the finite-family primitive from the shared
  finite-chain core and then applies the ordinary finite-family trace-MGF
  wrapper. The private core avoids proof duplication, while the exported
  signature keeps the ordinary finite-family assumptions for API consistency.
- Still unproved: the arbitrary finite-index finite-family Tropp/Lieb
  provider and automatic history/state construction.
...
- RM-TROPP-S7 records no downstream Matrix Bernstein conditional-step wrapper:
  the signature would be worse than the finite-family primitive route.
- S8 synchronizes examples/docs status without changing examples.
...
- RM-negative-square-integrability-adapters is complete.
- RM-negative-exp-trace-primitive-audit is complete.
- RM-negative-tropp-primitive-boundary-audit is complete.
- Separate local leaf: RM-BR-natural-history-state-construction.
- Next safe task: RM-negative-trace-mgf-provider-wrapper-audit.
...
- Still unproved: arbitrary finite-index finite-family Tropp/Lieb provider,
  Bernstein CFC primitive, Lieb, Golden-Thompson, and Matrix Bernstein tail
  theorem.
- RM-VP variance-proxy control is complete and proves the crude pointwise
  operator-norm route.
- S6 deterministic rank-one nullspace API is complete.
...
- S7E/S7F conditional sample-covariance operator-norm API remains available,
  ...
  positive-threshold arbitrary wrapper.
- RM-TROPP-S7 records no downstream Matrix Bernstein conditional-step wrapper:
  the signature would be worse than the finite-family primitive route. S8
  synchronizes examples/docs status without changing examples.
- RM-BR state prefix/suffix partial-sum API is complete and provides only
  finite-sum/state bookkeeping for the next natural-state construction leaf.
- RM-negative-square-integrability-adapters is complete.
- RM-negative-exp-trace-primitive-audit is complete.
