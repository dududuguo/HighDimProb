...
real-to-lintegral trace-MGF bridge, the explicit-theta one-sided
quadratic-form upper-tail wrappers, the scalar dimension/norm RHS reduction,
and the theta-optimized scalar RHS wrapper
...
are also proved; they keep the general self-adjoint operator-norm spectral
bridge assumption and all variance-proxy / primitive assumptions explicit.
...
The current arbitrary-dimensional operator-norm API uses this positive-threshold
route and does not package the zero-dimensional `t = 0` endpoint.
...
Golden-Thompson/Lieb proofs, Bernstein CFC proof, and a packaged `t = 0`
operator-norm endpoint remain outside the current API.
...
Next safe task: `RM-BR-natural-history-state-construction`. The negative
square-integrability adapter is complete; negative exp/trace/CFC/Tropp
...
- `lambdaMax_is_greatest_eigenvalue_statement` and
  `lambdaMin_is_least_eigenvalue_statement` remain typed targets. The general
  `selfAdjointOperatorNormTailViaQuadraticFormStatement` remains a typed
  target, while
  `selfAdjointOperatorNormTailViaQuadraticFormStatement_nonempty` proves its
  nonempty `Fin (n + 1)` instance.
...
The one-step `troppMasterTraceMGFStep_statement` remains available. Later
RM-TROPP cleanup proves only the narrow `Fin m` conditional-step provider and
conditional-step trace-MGF wrapper; the arbitrary finite-index provider,
Lieb, Golden-Thompson, and Matrix Bernstein theorem remain open.
...
These wrappers only apply explicit finite-family primitive assumptions. Later
RM-TROPP cleanup proves only the narrow `Fin m` conditional-step provider and
conditional-step trace-MGF wrapper. The arbitrary finite-index Tropp provider
and Bernstein CFC primitive remain open, and no Lieb, Golden-Thompson, or
...
`bernsteinMatrixExp_le_quadratic_statement`. Later RM-TROPP cleanup proves a
narrow `Fin m` provider from conditional-step/state data, but the arbitrary
finite-index provider remains open. It does not prove Tropp/Lieb,
