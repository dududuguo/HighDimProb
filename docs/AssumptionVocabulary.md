...
- Operator-norm measurability, centered operator-norm contraction, named
  ...
  family adapter branch, negative exp/trace sign-normalization audit, and
  negative Tropp primitive boundary audit are complete. The next RandomMatrix
  branch is `RM-negative-trace-mgf-provider-wrapper-audit`.
