...
- Mathlib's ordered endpoint control is available for
  `Matrix.IsHermitian.eigenvaluesâ‚€`, but current HighDimProb `lambdaMax` uses
...
- Preserved the existing public `lambdaMax` API and added the separate ordered
  endpoint wrapper `lambdaMaxOrdered`, defined directly as `hA.eigenvaluesâ‚€ 0`.
- Proved `lambdaMaxOrdered_eq_eigenvaluesâ‚€_zero` and
