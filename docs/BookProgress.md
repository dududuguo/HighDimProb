...
without an explicit spectral-bridge assumption. RM-negative-tropp-primitive-boundary-audit complete: the public API now includes
second-moment, variance-proxy, Bernstein K-family, per-summand MGF-comparison,
and post-provider bounded trace-MGF sign-normalization adapters for
`negRandomMatrixFamily`. These expose original-family `-theta` provider
...
RM-negative-square-integrability-adapters is complete. Current next safe task: RM-negative-trace-mgf-provider-wrapper-audit.
...
cleanup is complete. RM-negative-square-integrability-adapters is complete. Current next safe task: RM-negative-trace-mgf-provider-wrapper-audit.
