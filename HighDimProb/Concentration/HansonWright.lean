...
* `HighDimProb.HansonWright.centeredQuadraticForm`: the centered random quadratic form.
* `HighDimProb.HansonWright.deterministicFrobeniusNorm`: the Frobenius norm of a
  finite real matrix.

## Main results

* `HighDimProb.HansonWright.hanson_wright_inequality`: Hanson-Wright tail bound
  after deriving the MGF certificate from independent sub-Gaussian coordinates.
* `HighDimProb.HansonWright.hanson_wright_inequality_hdp`: Hanson-Wright tail
  bound using a universal constant and the maximum coordinate ψ₂ scale.
-/
...
namespace HighDimProb.HansonWright

variable {Ω : Type*} [MeasurableSpace Ω]

private lemma real_inner_mul (a b : ℝ) : inner ℝ a b = a * b := by
  calc
...
private lemma mgf_innerSL_stdGaussian {E : Type*} [NormedAddCommGroup E] [InnerProductSpace ℝ E]
...
private lemma integrable_exp_innerSL_stdGaussian {E : Type*} [NormedAddCommGroup E]
...
private lemma integral_exp_innerSL_stdGaussian {E : Type*} [NormedAddCommGroup E]
    [InnerProductSpace ℝ E] [FiniteDimensional ℝ E] [MeasurableSpace E] [BorelSpace E]
    (v : E) (t : ℝ) :
...
private lemma hasSubgaussianMGF_id_gaussianReal_zero_one :
...
private lemma symmetric_inner_apply_eq_sum_eigenvalues_repr {E : Type*} [NormedAddCommGroup E]
...
private lemma orthonormalBasis_repr_sum_smul {E : Type*} [NormedAddCommGroup E]
    [InnerProductSpace ℝ E] [FiniteDimensional ℝ E]
...
private def randomQuadraticForm {n : ℕ} (A : Matrix (Fin n) (Fin n) ℝ)
    (X : Fin n → Ω → ℝ) : Ω → ℝ :=
  fun ω => matrixQuadraticForm A fun i => X i ω

/-- The coordinate random vector as an element of Euclidean space. -/
private def randomVector {n : ℕ} (X : Fin n → Ω → ℝ) : Ω → EuclideanSpace ℝ (Fin n) :=
  fun ω => WithLp.toLp 2 fun i => X i ω

omit [MeasurableSpace Ω] in
@[simp]
private lemma randomVector_apply {n : ℕ} (X : Fin n → Ω → ℝ) (ω : Ω) (i : Fin n) :
    randomVector X ω i = X i ω := rfl

private lemma randomVector_aemeasurable {μ : Measure Ω} {n : ℕ} {X : Fin n → Ω → ℝ}
...
omit [MeasurableSpace Ω] in
private lemma measure_map_prod_map_of_aemeasurable {α β γ δ : Type*}
...
private def frobeniusNormSq {n : ℕ} (A : Matrix (Fin n) (Fin n) ℝ) : ℝ :=
  ∑ i, ∑ j, (A i j) ^ 2
...
private def subtypeMask {n : ℕ} (s : Finset (Fin n)) (x : s → ℝ) :
    EuclideanSpace ℝ (Fin n) :=
  WithLp.toLp 2 fun i => if h : i ∈ s then x ⟨i, h⟩ else 0

@[simp]
private lemma subtypeMask_apply {n : ℕ} (s : Finset (Fin n)) (x : s → ℝ) (i : Fin n) :
    subtypeMask s x i = if h : i ∈ s then x ⟨i, h⟩ else 0 := rfl

private lemma measurable_subtypeMask {n : ℕ} (s : Finset (Fin n)) :
...
omit [MeasurableSpace Ω] in
private lemma subtypeMask_subtype_randomVector {n : ℕ} (s : Finset (Fin n))
    (X : Fin n → Ω → ℝ) :
...
private lemma coordinateMask_aemeasurable {μ : Measure Ω} {n : ℕ} (s : Finset (Fin n))
...
private lemma coordinateMask_indepFun_compl {μ : Measure Ω} {n : ℕ} {X : Fin n → Ω → ℝ}
...
private lemma frobeniusNormSq_nonneg {n : ℕ} (A : Matrix (Fin n) (Fin n) ℝ) :
    0 ≤ frobeniusNormSq A := by
  unfold frobeniusNormSq
  exact Finset.sum_nonneg fun _ _ => Finset.sum_nonneg fun _ _ => sq_nonneg _

private lemma frobeniusNorm_nonneg {n : ℕ} (A : Matrix (Fin n) (Fin n) ℝ) :
    0 ≤ deterministicFrobeniusNorm A :=
  sqrt_nonneg _

private lemma operatorNorm_nonneg {n : ℕ} (A : Matrix (Fin n) (Fin n) ℝ) :
    0 ≤ deterministicOperatorNorm A :=
  norm_nonneg _

private lemma entrywiseL1Norm_nonneg {n : ℕ} (A : Matrix (Fin n) (Fin n) ℝ) :
    0 ≤ entrywiseL1Norm A := by
  unfold entrywiseL1Norm
  exact Finset.sum_nonneg fun _ _ => Finset.sum_nonneg fun _ _ => abs_nonneg _

private lemma coordinateMask_norm_le {n : ℕ} (s : Finset (Fin n))
...
private lemma toEuclideanCLM_cutMatrix_apply {n : ℕ} (A : Matrix (Fin n) (Fin n) ℝ)
...
private lemma inner_toEuclideanCLM_cutMatrix_eq_inner_masks {n : ℕ}
...
private lemma operatorNorm_cutMatrix_le {n : ℕ} (A : Matrix (Fin n) (Fin n) ℝ)
    (s : Finset (Fin n)) :
...
private lemma frobeniusNormSq_cutMatrix_le {n : ℕ} (A : Matrix (Fin n) (Fin n) ℝ)
    (s : Finset (Fin n)) :
...
private lemma frobeniusNorm_sq {n : ℕ} (A : Matrix (Fin n) (Fin n) ℝ) :
    deterministicFrobeniusNorm A ^ 2 = frobeniusNormSq A := by
  unfold deterministicFrobeniusNorm
  rw [sq_sqrt (frobeniusNormSq_nonneg A)]

private lemma frobeniusNorm_cutMatrix_sq_le {n : ℕ} (A : Matrix (Fin n) (Fin n) ℝ)
    (s : Finset (Fin n)) :
...
private lemma card_powerset_filter_mem_notMem_eq {n : ℕ} {i j : Fin n} (hij : i ≠ j) :
...
private lemma four_mul_card_powerset_filter_mem_notMem {n : ℕ} {i j : Fin n} (hij : i ≠ j) :
...
private lemma sum_powerset_cut_indicator {n : ℕ} (i j : Fin n) (a : ℝ) :
...
private lemma four_mul_sum_cut_quadraticForm_eq_card_mul_offDiagonal {n : ℕ}
...
private lemma quadraticForm_offDiagonal_eq_average_cut {n : ℕ}
    (A : Matrix (Fin n) (Fin n) ℝ) (x : Fin n → ℝ) :
...
private lemma exp_mul_quadraticForm_offDiagonal_le_average_cut {n : ℕ}
...
private lemma quadraticForm_eq_inner_toEuclideanCLM {n : ℕ}
    (A : Matrix (Fin n) (Fin n) ℝ) (x : Fin n → ℝ) :
...
private lemma sum_diag_sq_le_frobeniusNormSq {n : ℕ} (A : Matrix (Fin n) (Fin n) ℝ) :
    (∑ i, A i i ^ 2) ≤ frobeniusNormSq A := by
  unfold frobeniusNormSq
  exact Finset.sum_le_sum fun i _ => by
    exact Finset.single_le_sum (fun j _ => sq_nonneg (A i j)) (Finset.mem_univ i)

private lemma sum_diag_sq_le_frobeniusNorm_sq {n : ℕ} (A : Matrix (Fin n) (Fin n) ℝ) :
    (∑ i, A i i ^ 2) ≤ deterministicFrobeniusNorm A ^ 2 := by
  rw [frobeniusNorm_sq]
  exact sum_diag_sq_le_frobeniusNormSq A

private lemma abs_diag_le_operatorNorm {n : ℕ} (A : Matrix (Fin n) (Fin n) ℝ) (i : Fin n) :
...
private lemma abs_quadraticForm_le {n : ℕ} (A : Matrix (Fin n) (Fin n) ℝ)
...
private lemma randomQuadraticForm_aemeasurable {μ : Measure Ω} {n : ℕ}
...
private lemma randomQuadraticForm_mem_Icc_of_ae_coord_bound {μ : Measure Ω} {n : ℕ}
...
private lemma bounded_hansonWright_cgf_constant_le {n : ℕ}
...
private def HasHansonWrightMGF {n : ℕ} (μ : Measure Ω) (A : Matrix (Fin n) (Fin n) ℝ)
...
private lemma exp_le_chord {R t x : ℝ} (hR : 0 < R) (hx : x ∈ Set.Icc (-R) R) :
...
private lemma hasSubgaussianMGF_of_abs_le_of_integral_eq_zero {μ : Measure Ω}
...
private lemma integral_cosh_mul_le_of_hasSubgaussianMGF {μ : Measure Ω} [IsProbabilityMeasure μ]
...
private lemma cosh_taylor_term_le (y : ℝ) (m : ℕ) :
...
private lemma integral_even_taylor_term_le_of_hasSubgaussianMGF {μ : Measure Ω}
...
private lemma integral_even_power_le_of_hasSubgaussianMGF {μ : Measure Ω}
    [IsProbabilityMeasure μ] {X : Ω → ℝ} {c : ℝ≥0}
...
private lemma exp_mul_sq_eq_tsum (θ x : ℝ) :
    exp (θ * x ^ 2) =
...
private lemma factorial_two_mul_div_factorial_le_succ_pow (m : ℕ) :
...
private lemma integral_exp_sq_series_term_le_of_hasSubgaussianMGF {μ : Measure Ω}
    [IsProbabilityMeasure μ] {X : Ω → ℝ} {c : ℝ≥0}
...
private lemma integral_exp_sq_series_term_le_of_hasSubgaussianMGF_of_le {μ : Measure Ω}
    [IsProbabilityMeasure μ] {X : Ω → ℝ} {c : ℝ≥0}
...
private lemma summable_integral_exp_sq_series_of_hasSubgaussianMGF {μ : Measure Ω}
    [IsProbabilityMeasure μ] {X : Ω → ℝ} {c : ℝ≥0}
...
private lemma summable_integral_exp_sq_series_of_hasSubgaussianMGF_of_le {μ : Measure Ω}
    [IsProbabilityMeasure μ] {X : Ω → ℝ} {c : ℝ≥0}
    (h : HasSubgaussianMGF X c μ) {θ C0 : ℝ} (hθ : 0 ≤ θ)
...
private lemma integral_exp_mul_sq_eq_tsum_integrals {μ : Measure Ω}
    [IsProbabilityMeasure μ] {X : Ω → ℝ} {c : ℝ≥0}
    (h : HasSubgaussianMGF X c μ) {θ : ℝ} (hθ : 0 ≤ θ)
...
private lemma integral_exp_mul_sq_le_inv_of_hasSubgaussianMGF_of_le {μ : Measure Ω}
    [IsProbabilityMeasure μ] {X : Ω → ℝ} {c : ℝ≥0}
    (h : HasSubgaussianMGF X c μ) {θ C0 : ℝ} (hθ : 0 ≤ θ)
    (hC0 : 0 < C0) (hc_le : (c : ℝ) ≤ C0)
    (hθ_small : θ * C0 * exp 1 < 1) :
...
private lemma integral_exp_mul_sq_le_one_add_tail_of_hasSubgaussianMGF_of_le {μ : Measure Ω}
    [IsProbabilityMeasure μ] {X : Ω → ℝ} {c : ℝ≥0}
    (h : HasSubgaussianMGF X c μ) {θ C0 : ℝ} (hθ : 0 ≤ θ)
    (hC0 : 0 < C0) (hc_le : (c : ℝ) ≤ C0)
    (hθ_small : θ * C0 * exp 1 < 1) :
...
private lemma integral_exp_mul_sq_le_one_add_linear_add_tail_of_hasSubgaussianMGF_of_le
...
private lemma integral_exp_mul_sq_le_exp_linear_of_hasSubgaussianMGF_of_le {μ : Measure Ω}
    [IsProbabilityMeasure μ] {X : Ω → ℝ} {c : ℝ≥0}
    (h : HasSubgaussianMGF X c μ) {θ C0 : ℝ} (hθ : 0 ≤ θ)
    (hC0 : 0 < C0) (hc_le : (c : ℝ) ≤ C0)
...
private lemma integral_exp_quadratic_stdGaussian_le {E : Type*} [NormedAddCommGroup E]
    [InnerProductSpace ℝ E] [FiniteDimensional ℝ E] [MeasurableSpace E] [BorelSpace E]
...
private lemma matrix_trace_conjTranspose_mul_self_eq_frobeniusNormSq {n : ℕ}
...
private lemma toEuclideanCLM_adjoint {n : ℕ} (A : Matrix (Fin n) (Fin n) ℝ) :
...
private lemma operatorNorm_conjTranspose {n : ℕ} (A : Matrix (Fin n) (Fin n) ℝ) :
    deterministicOperatorNorm A.conjTranspose = deterministicOperatorNorm A := by
  unfold deterministicOperatorNorm
  exact Matrix.l2_opNorm_conjTranspose A

private lemma frobeniusNormSq_conjTranspose {n : ℕ} (A : Matrix (Fin n) (Fin n) ℝ) :
    frobeniusNormSq A.conjTranspose = frobeniusNormSq A := by
  unfold frobeniusNormSq
  simp only [Matrix.conjTranspose_apply, star_trivial]
  rw [Finset.sum_comm]

private lemma frobeniusNorm_conjTranspose_sq {n : ℕ} (A : Matrix (Fin n) (Fin n) ℝ) :
...
private lemma trace_adjoint_comp_toEuclideanCLM_eq_frobeniusNormSq {n : ℕ}
    (A : Matrix (Fin n) (Fin n) ℝ) :
...
private lemma eigenvalue_adjoint_comp_le_opNorm_sq {E : Type*} [NormedAddCommGroup E]
    [InnerProductSpace ℝ E] [FiniteDimensional ℝ E]
...
private lemma integral_sq_le_of_hasSubgaussianMGF_of_le {μ : Measure Ω}
    [IsProbabilityMeasure μ] {X : Ω → ℝ} {c : ℝ≥0}
...
private lemma integral_fourth_le_of_hasSubgaussianMGF_of_le {μ : Measure Ω}
    [IsProbabilityMeasure μ] {X : Ω → ℝ} {c : ℝ≥0}
    (h : HasSubgaussianMGF X c μ) {C0 : ℝ}
    (hC0 : 0 < C0) (hc_le : (c : ℝ) ≤ C0) :
...
private lemma exp_neg_le_one_sub_add_sq {u : ℝ} (hu : 0 ≤ u) :
    exp (-u) ≤ 1 - u + u ^ 2 := by
...
private lemma exp_neg_mul_one_add_le_one (u : ℝ) :
    exp (-u) * (1 + u) ≤ 1 := by
  have h_exp_ge : 1 + u ≤ exp u := by
    simpa [add_comm] using Real.add_one_le_exp u
  calc
...
private lemma integral_exp_centered_sq_le_exp_quadratic_nonneg {μ : Measure Ω}
    [IsProbabilityMeasure μ] {X : Ω → ℝ} {c : ℝ≥0}
    (h : HasSubgaussianMGF X c μ) {θ C0 : ℝ} (hθ : 0 ≤ θ)
    (hC0 : 0 < C0) (hc_le : (c : ℝ) ≤ C0)
    (hθ_half : θ * C0 * exp 1 ≤ 1 / 2) :
...
private lemma integral_exp_centered_sq_le_exp_quadratic_nonpos {μ : Measure Ω}
    [IsProbabilityMeasure μ] {X : Ω → ℝ} {c : ℝ≥0}
...
private lemma integrable_exp_mul_sq_of_summable_integrals {μ : Measure Ω}
    [IsProbabilityMeasure μ] {X : Ω → ℝ} {c : ℝ≥0}
    (h : HasSubgaussianMGF X c μ) {θ : ℝ} (hθ : 0 ≤ θ)
    (hsum : Summable fun m : ℕ =>
      ∫ ω, θ ^ m * X ω ^ (2 * m) / (Nat.factorial m : ℝ) ∂μ) :
...
private lemma integrable_exp_mul_sq_of_hasSubgaussianMGF {μ : Measure Ω}
    [IsProbabilityMeasure μ] {X : Ω → ℝ} {c : ℝ≥0}
    (h : HasSubgaussianMGF X c μ) {θ : ℝ} (hθ : 0 ≤ θ)
    (hθ_small : θ * ((c : ℝ) + 1) * exp 1 < 1) :
...
private lemma integrable_exp_mul_sq_of_hasSubgaussianMGF_of_le {μ : Measure Ω}
    [IsProbabilityMeasure μ] {X : Ω → ℝ} {c : ℝ≥0}
    (h : HasSubgaussianMGF X c μ) {θ C0 : ℝ} (hθ : 0 ≤ θ)
    (hC0 : 0 < C0) (hc_le : (c : ℝ) ≤ C0)
    (hθ_small : θ * C0 * exp 1 < 1) :
...
private lemma integrable_exp_quadratic_stdGaussian {E : Type*} [NormedAddCommGroup E]
    [InnerProductSpace ℝ E] [FiniteDimensional ℝ E] [MeasurableSpace E] [BorelSpace E]
    (T : E →L[ℝ] E) (hTpos : T.toLinearMap.IsPositive) {θ : ℝ} (hθ : 0 ≤ θ)
    (hsmall : ∀ i : Fin (Module.finrank ℝ E),
...
private lemma integrable_exp_inner_toEuclideanCLM_prod_stdGaussian {n : ℕ}
  ...
  have hsmall_eig :
    ...
    exact lt_of_le_of_lt hstep (by simpa [deterministicOperatorNorm, T] using hsmall)
  have hquad_int :
...
private lemma integral_exp_inner_toEuclideanCLM_prod_stdGaussian_le {n : ℕ}
    (A : Matrix (Fin n) (Fin n) ℝ) {l : ℝ}
  ...
  have hsmall_eig :
    ...
    have hstep :
      ...
      exact mul_le_mul_of_nonneg_right
        (mul_le_mul_of_nonneg_left heig_le hcoef_nonneg) (exp_nonneg 1)
    exact hstep.trans (by simpa [deterministicOperatorNorm, T] using hsmall)
  have hsum_eq :
...
private lemma integrable_exp_centered_sq_of_hasSubgaussianMGF_of_le {μ : Measure Ω}
    [IsProbabilityMeasure μ] {X : Ω → ℝ} {c : ℝ≥0}
...
private lemma mgf_id_map_eq {μ : Measure Ω} {X : Ω → ℝ}
...
private lemma integrable_exp_mul_id_map {μ : Measure Ω} {X : Ω → ℝ}
...
private lemma hasSubgaussianMGF_id_map {μ : Measure Ω} {X : Ω → ℝ} {c : ℝ≥0}
...
private lemma integrable_exp_mul_snd_fst_prod_of_hasSubgaussianMGF_of_le
...
private lemma integral_exp_mul_snd_fst_prod_le_of_hasSubgaussianMGF_of_le
    {νX νY : Measure ℝ} [IsProbabilityMeasure νX] [IsProbabilityMeasure νY]
    {cX cY : ℝ≥0} (hX : HasSubgaussianMGF id cX νX)
    (hY : HasSubgaussianMGF id cY νY) {C0 θ : ℝ}
    (hC0 : 0 < C0) (hcX : (cX : ℝ) ≤ C0) (hcY : (cY : ℝ) ≤ C0)
...
private lemma integrable_exp_mul_prod_of_indepFun_hasSubgaussianMGF_of_le
...
private lemma integral_exp_mul_prod_le_of_indepFun_hasSubgaussianMGF_of_le
    {μ : Measure Ω} [IsProbabilityMeasure μ] {X Y : Ω → ℝ}
    (h_indep : X ⟂ᵢ[μ] Y)
    {cX cY : ℝ≥0} (hX : HasSubgaussianMGF X cX μ)
    (hY : HasSubgaussianMGF Y cY μ) {C0 θ : ℝ}
...
private lemma centered_sq_cgf_le_of_hasSubgaussianMGF_of_le {μ : Measure Ω}
    [IsProbabilityMeasure μ] {X : Ω → ℝ} {c : ℝ≥0}
    (h : HasSubgaussianMGF X c μ) {θ C0 : ℝ}
    (hC0 : 0 < C0) (hc_le : (c : ℝ) ≤ C0)
...
private def diagonalCenteredQuadraticForm {n : ℕ} (μ : Measure Ω)
    (A : Matrix (Fin n) (Fin n) ℝ) (X : Fin n → Ω → ℝ) : Ω → ℝ :=
  fun ω => ∑ i, A i i * (X i ω ^ 2 - ∫ ω, X i ω ^ 2 ∂μ)

private lemma cgf_const_mul {μ : Measure Ω} {Y : Ω → ℝ} (a θ : ℝ) :
    cgf (fun ω => a * Y ω) μ θ = cgf Y μ (a * θ) := by
  rw [cgf, cgf, mgf_const_mul]

private lemma iIndepFun_centered_sq {μ : Measure Ω} {n : ℕ} {X : Fin n → Ω → ℝ}
...
private lemma iIndepFun.integrable_exp_mul_sum₀ {ι : Type*} {μ : Measure Ω} [IsFiniteMeasure μ]
    {X : ι → Ω → ℝ}
...
private lemma diagonalCenteredQuadraticForm_cgf_le {μ : Measure Ω} [IsProbabilityMeasure μ]
...
private lemma diagonalCenteredQuadraticForm_integrable_exp {μ : Measure Ω} [IsProbabilityMeasure μ]
    {n : ℕ} {A : Matrix (Fin n) (Fin n) ℝ} {X : Fin n → Ω → ℝ}
    {K C θ : ℝ} (hK : 0 < K) (hC : 0 < C)
...
private lemma quadraticForm_eq_sum_diag_of_offdiag_eq_zero {n : ℕ}
    {A : Matrix (Fin n) (Fin n) ℝ} {x : Fin n → ℝ}
...
private lemma centeredQuadraticForm_eq_diagonalCentered_of_offdiag_eq_zero
...
private theorem hasHansonWrightMGF_diagonal_of_subgaussian {μ : Measure Ω} [IsProbabilityMeasure μ]
    {n : ℕ} {A : Matrix (Fin n) (Fin n) ℝ} {X : Fin n → Ω → ℝ}
    {K C : ℝ} (hK : 0 < K) (hC : 0 < C)
    (hC_domain : exp 1 ≤ C) (hC_quad : 2 * exp 1 ^ 3 ≤ C)
    (hOp : 0 < deterministicOperatorNorm A)
...
private lemma mean_le_log_mgf {μ : Measure Ω} [IsProbabilityMeasure μ]
...
private lemma hasSubgaussianMGF_integral_eq_zero {μ : Measure Ω} [IsProbabilityMeasure μ]
    {X : Ω → ℝ} {c : ℝ≥0} (h : HasSubgaussianMGF X c μ) :
    ∫ ω, X ω ∂μ = 0 := by
...
private lemma quadraticForm_eq_diag_add_offDiagonal {n : ℕ}
    (A : Matrix (Fin n) (Fin n) ℝ) (x : Fin n → ℝ) :
    matrixQuadraticForm A x =
...
private lemma integrable_randomQuadraticForm_offDiagonal_of_subgaussian {μ : Measure Ω}
...
private lemma integral_randomQuadraticForm_offDiagonal_eq_zero {μ : Measure Ω}
    [IsProbabilityMeasure μ] {n : ℕ} (A : Matrix (Fin n) (Fin n) ℝ)
    {X : Fin n → Ω → ℝ} {K : ℝ}
    (h_indep : iIndepFun X μ)
    (hX_subG : ∀ i, HasSubgaussianMGF (X i) ⟨K ^ 2, sq_nonneg K⟩ μ) :
  ...
  have hrow_int : ∀ i,
    ...
    intro i
    exact MeasureTheory.integrable_finset_sum _ fun j _ => hterm_int i j
  calc
    ∫ ω, randomQuadraticForm (offDiagonalMatrix A) X ω ∂μ
        = ∑ i, ∑ j,
            ∫ ω, X i ω * offDiagonalMatrix A i j * X j ω ∂μ := by
          unfold randomQuadraticForm matrixQuadraticForm
          rw [MeasureTheory.integral_finset_sum _ (fun i _ => hrow_int i)]
          apply Finset.sum_congr rfl
          intro i _
          rw [MeasureTheory.integral_finset_sum _ (fun j _ => hterm_int i j)]
    _ = 0 := by
...
private lemma centeredQuadraticForm_eq_diagonal_add_offDiagonal {μ : Measure Ω}
    [IsProbabilityMeasure μ] {n : ℕ} (A : Matrix (Fin n) (Fin n) ℝ)
    {X : Fin n → Ω → ℝ} {K : ℝ}
    (h_indep : iIndepFun X μ)
    (hX_subG : ∀ i, HasSubgaussianMGF (X i) ⟨K ^ 2, sq_nonneg K⟩ μ) :
  ...
  have hdiag_int :
    ...
    MeasureTheory.integrable_finset_sum _ fun i _ => hdiag_term_int i
  have hoff_int :
...
private lemma hasSubgaussianMGF_mono_param {μ : Measure Ω} {X : Ω → ℝ} {c d : ℝ≥0}
...
private lemma hasSubgaussianMGF_finset_sum_const_mul_of_iIndepFun {ι : Type*} {μ : Measure Ω}
...
private lemma inner_randomVector_hasSubgaussianMGF_of_iIndepFun {μ : Measure Ω}
    {n : ℕ} {X : Fin n → Ω → ℝ} {K : ℝ}
    (h_indep : iIndepFun X μ)
    (hX_subG : ∀ i, HasSubgaussianMGF (X i) ⟨K ^ 2, sq_nonneg K⟩ μ)
...
sub-Gaussian vector. -/
private lemma integral_exp_norm_toEuclideanCLM_randomVector_sq_le {μ : Measure Ω}
    [IsProbabilityMeasure μ] {n : ℕ} (A : Matrix (Fin n) (Fin n) ℝ)
...
private lemma integrable_exp_norm_toEuclideanCLM_randomVector_sq {μ : Measure Ω}
    [IsProbabilityMeasure μ] {n : ℕ} (A : Matrix (Fin n) (Fin n) ℝ)
    {X : Fin n → Ω → ℝ} {K θ : ℝ} (hθ : 0 ≤ θ)
    (h_indep : iIndepFun X μ)
    (hX_subG : ∀ i, HasSubgaussianMGF (X i) ⟨K ^ 2, sq_nonneg K⟩ μ)
...
private lemma integrable_exp_inner_toEuclideanCLM_randomVector_prod {μ : Measure Ω}
    [IsProbabilityMeasure μ] {n : ℕ} (A : Matrix (Fin n) (Fin n) ℝ)
    {X : Fin n → Ω → ℝ} {K l : ℝ}
    (h_indep : iIndepFun X μ)
    (hX_subG : ∀ i, HasSubgaussianMGF (X i) ⟨K ^ 2, sq_nonneg K⟩ μ)
...
private lemma integral_exp_inner_toEuclideanCLM_randomVector_prod_le {μ : Measure Ω}
    [IsProbabilityMeasure μ] {n : ℕ} (A : Matrix (Fin n) (Fin n) ℝ)
    {X : Fin n → Ω → ℝ} {K l : ℝ}
    (h_indep : iIndepFun X μ)
    (hX_subG : ∀ i, HasSubgaussianMGF (X i) ⟨K ^ 2, sq_nonneg K⟩ μ)
...
private lemma integral_exp_quadraticForm_cutMatrix_eq_prod {μ : Measure Ω}
    [IsProbabilityMeasure μ] {n : ℕ} (A : Matrix (Fin n) (Fin n) ℝ)
...
private lemma integrable_exp_quadraticForm_cutMatrix {μ : Measure Ω}
    [IsProbabilityMeasure μ] {n : ℕ} (A : Matrix (Fin n) (Fin n) ℝ)
...
private lemma integral_exp_quadraticForm_cutMatrix_le {μ : Measure Ω}
    [IsProbabilityMeasure μ] {n : ℕ} (A : Matrix (Fin n) (Fin n) ℝ)
    (s : Finset (Fin n)) {X : Fin n → Ω → ℝ} {K l : ℝ}
    (h_indep : iIndepFun X μ)
    (hX_subG : ∀ i, HasSubgaussianMGF (X i) ⟨K ^ 2, sq_nonneg K⟩ μ)
...
private lemma integrable_exp_quadraticForm_offDiagonal {μ : Measure Ω}
    [IsProbabilityMeasure μ] {n : ℕ} (A : Matrix (Fin n) (Fin n) ℝ)
    {X : Fin n → Ω → ℝ} {K l : ℝ}
    (h_indep : iIndepFun X μ)
    (hX_subG : ∀ i, HasSubgaussianMGF (X i) ⟨K ^ 2, sq_nonneg K⟩ μ)
  ...
  have hR_int : Integrable R μ := by
    dsimp [R]
    refine (MeasureTheory.integrable_finset_sum P fun s hs => ?_).const_mul _
...
private lemma integral_exp_quadraticForm_offDiagonal_le {μ : Measure Ω}
    [IsProbabilityMeasure μ] {n : ℕ} (A : Matrix (Fin n) (Fin n) ℝ)
    {X : Fin n → Ω → ℝ} {K l : ℝ}
    (h_indep : iIndepFun X μ)
    (hX_subG : ∀ i, HasSubgaussianMGF (X i) ⟨K ^ 2, sq_nonneg K⟩ μ)
  ...
  have hR_int : Integrable R μ := by
    dsimp [R]
    exact (MeasureTheory.integrable_finset_sum P hcut_int).const_mul _
  have hleft_int :
      Integrable
        (fun ω => exp (l * matrixQuadraticForm (offDiagonalMatrix A) (fun i => X i ω))) μ :=
...
private lemma exp_add_le_average_exp_two (a b : ℝ) :
...
private lemma abs_two_mul_le_inv_quarter_of_abs_le {a C l : ℝ}
    (ha : 0 < a) (hC : 0 < C)
    (hl : |l| ≤ (2 * C * a)⁻¹) :
    |2 * l| ≤ (2 * (C / 4) * a)⁻¹ := by
...
private lemma abs_mul_le_inv_two_of_abs_le {a C l : ℝ}
    (ha : 0 < a) (hC : 0 < C)
    (hl : |l| ≤ (2 * C * a)⁻¹) :
    |l| * a ≤ 1 / (2 * C) := by
  calc
...
private lemma offDiagonal_small_two_mul_of_abs_le {K C op l : ℝ}
...
private lemma offDiagonal_exponent_two_mul_le {K C F l : ℝ}
...
private theorem hasHansonWrightMGF_of_subgaussian {μ : Measure Ω} [IsProbabilityMeasure μ]
    {n : ℕ} {A : Matrix (Fin n) (Fin n) ℝ} {X : Fin n → Ω → ℝ}
    {K C : ℝ} (hK : 0 < K) (hC : 0 < C)
    (hC_domain : 4 * exp 1 ≤ C)
...
private theorem hasHansonWrightMGF_of_bounded {μ : Measure Ω} [IsProbabilityMeasure μ]
    {n : ℕ} {A : Matrix (Fin n) (Fin n) ℝ} {X : Fin n → Ω → ℝ}
...
private lemma chernoff_bound_cgf {μ : Measure Ω} [IsProbabilityMeasure μ]
    {Y : Ω → ℝ} {l t : ℝ} (hl : 0 ≤ l)
...
private theorem two_sided_tail_of_cgf_bound {μ : Measure Ω} [IsProbabilityMeasure μ]
    {Y : Ω → ℝ} {v b C t : ℝ} (hC : 0 < C) (hv : 0 < v) (hb : 0 < b)
    (hcgf : ∀ l : ℝ, |l| ≤ (2 * C * b)⁻¹ → cgf Y μ l ≤ C * l ^ 2 * v)
    (hint : ∀ l : ℝ, |l| ≤ (2 * C * b)⁻¹ →
      Integrable (fun ω => exp (l * Y ω)) μ) (ht : 0 ≤ t) :
...
private theorem hanson_wright_inequality_hdp_explicit {μ : Measure Ω} [IsProbabilityMeasure μ]
    {n : ℕ} {A : Matrix (Fin n) (Fin n) ℝ} {X : Fin n → Ω → ℝ}
    {K C t : ℝ} (hK : 0 < K)
...
/-- Hanson-Wright inequality with a universal explicit constant for direct
coordinate sub-Gaussian MGF assumptions. -/
