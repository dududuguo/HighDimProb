...
| `HighDimProb.Dudley.Inputs.bound` | 全有界索引集上的完整锚定 Dudley 熵积分界；有限网、父映射和极限可积性论证由 consumer 内部完成。 | [源码](HighDimProb/Concentration/DudleyFull.lean) · [示例](HighDimProb/Examples/DudleyUsage.lean) · [Judge](HighDimProbJudge/Concentration/DudleyUse.lean) |
| `HighDimProb.dudleyEntropyIntegral` | 在显式稠密序列、连续样本路径、上确界有界性和完整可积性假设下，给出另一条完整锚定 Dudley 熵积分界。 | [源码](HighDimProb/Concentration/Dudley.lean) · [API 测试](HighDimProbTest/DudleyEntropyIntegralAPI.lean) |
